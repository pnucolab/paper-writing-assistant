<script lang="ts">
	...
	import DeleteDraftModal from '$lib/components/drafts/DeleteDraftModal.svelte';
	...
	let draftToDelete = $state<Draft | null>(null);
	...
	function confirmDelete() {
		...
		// Filter creates a new array, which should trigger reactivity
		drafts = [...drafts.filter((d) => d.id !== draftToDelete.id)];
		saveDrafts();

		// Clean up localStorage for this draft
		try {
			localStorage.removeItem(`paperwriter-draft-${draftToDelete.id}-documents`);
			localStorage.removeItem(`paperwriter-draft-${draftToDelete.id}-focus`);
			localStorage.removeItem(`paperwriter-draft-${draftToDelete.id}-outline`);
			localStorage.removeItem(`paperwriter-draft-${draftToDelete.id}-format`);
		} catch (error) {
	...
	function getStepDisplayName(step: WorkflowStep, isCompleted?: boolean): string {
		...
		switch (step) {
			case 'format':
				return m.newpaper_step_format_title();
			case 'documents':
				return m.newpaper_step_documents_title();
			case 'focus':
				return m.newpaper_step_focus_title();
			case 'outline':
				return m.newpaper_step_outline_title();
			case 'writing':
				return m.newpaper_step_writing_title();
			default:
				return step;
		}
	}
...
<DeleteDraftModal
	...
	draftTitle={draftToDelete?.projectTitle || ''}
